# Developer and Research Guide

This page contains the framework, Python, training, and research documentation inherited from
[DeepFilterNet](https://github.com/Rikorose/DeepFilterNet). For prebuilt downloads and
end-user setup, start with the [main README](../README.md).

DeepFilterNet is a low-complexity speech enhancement framework for full-band audio based on deep filtering.
...
DeepFilterNet Plus keeps the LADSPA library name (`libdeep_filter_ladspa.so`) and plugin identifiers
unchanged, so it remains a drop-in replacement for EasyEffects and PipeWire filter-chain setups.

- **Real-time safe LADSPA plugin** ([upstream issue #661](https://github.com/Rikorose/DeepFilterNet/issues/661)):
  `run()` no longer blocks the host's real-time thread. Latency adapts automatically, and overload
  falls back safely instead of crashing the audio server.
- **Stereo inference fix:** multi-channel models no longer panic with `tract` 0.21.5 and newer. The
  worker averages channel estimates and falls back to unprocessed passthrough if it stops.
- **Updated dependencies and upstream work:** includes upstream pull requests
  [#645](https://github.com/Rikorose/DeepFilterNet/pull/645),
  [#648](https://github.com/Rikorose/DeepFilterNet/pull/648),
  [#652](https://github.com/Rikorose/DeepFilterNet/pull/652),
  [#653](https://github.com/Rikorose/DeepFilterNet/pull/653),
  [#666](https://github.com/Rikorose/DeepFilterNet/pull/666),
  [#676](https://github.com/Rikorose/DeepFilterNet/pull/676), and
  [#685](https://github.com/Rikorose/DeepFilterNet/pull/685).
